...
public class BgUpdate {
    ...
    private static ServiceConnection serviceConnection = new
            ServiceConnection() {
                @Override
     ...
     */
    public static void updateForDialog(Context context,
                                       String url,
                                       String filePath){
        TYPE = TYPE_DIALOG;
        BgUpdate.url = url;
     ...
     * @param filePath
     */
    public static void updateForNotification(Context context,
                                             String url,
                                             String filePath){
    ...
    private static void startDownload(Context context){
        if(myBinder != null){
            ...
            if(Build.VERSION.SDK_INT >= Build
                    .VERSION_CODES.M){
                // 检查该权限是否已经获取
                int i = ContextCompat.checkSelfPermission(context,
                        permissions[0]);
